...
app.use(
  "/images",
  express.static(
    path.join(__dirname, "static/games/slots/triple-diamond/style/images")
  )
);
// Cross-game CSS asset references
app.use(
  "/book-of-winners",
...
// Symbols 10-15 = jokers (wild) - excluded from line wins. No scatter in this game.
const WIN_TABLE = {
  1: { 2: 10, 3: 100, 4: 1000, 5: 5000 },
  2: { 3: 40, 4: 400, 5: 2000 },
  3: { 3: 25, 4: 200, 5: 750 },
  4: { 3: 25, 4: 200, 5: 750 },
  5: { 3: 10, 4: 30, 5: 150 },
...
const SCATTER_SYMBOL = null; // no scatter in this game
const JOKER_SYMBOLS = [10, 11, 12, 13, 14, 15]; // wild symbols
const SPECIAL_SYMBOLS = [10, 11, 12, 13, 14, 15]; // jokers: max 1 per column
...
function generateSpinResponse(params) {
  ...
  for (let lineIdx = 0; lineIdx < lines && lineIdx < PAYLINES.length; lineIdx++) {
    ...
    // Skip joker/wild symbols - they don't start line wins on their own
    if (JOKER_SYMBOLS.includes(firstSym)) continue;
  ...
  // NOTE: No scatter in this game, so no scatter wins to check.
...
app.listen(PORT, () => {
  console.log(`\n  Triple Diamond running at http://localhost:${PORT}\n`);
