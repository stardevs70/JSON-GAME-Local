...
app.use(
  "/images",
  express.static(
    path.join(__dirname, "static/games/slots/fire-rage/style/images")
  )
);
...
// Fire Rage gamble card images from other game folders
app.use(
  "/sevens-on-fire",
  express.static(path.join(__dirname, "static/games/slots/sevens-on-fire"))
);
app.use(
  "/roll-of-ramses",
...
// Fire Rage is a 3-reel, 3-row game with 17 paylines
// Each array = [reel1_row, reel2_row, reel3_row], Row 1=top, 2=middle, 3=bottom
const NUM_REELS = 3;
const NUM_SYMBOLS = 9; // 9 symbols in Fire Rage
const PAYLINES = [
  [2, 2, 2], // line 1  - middle
  [1, 1, 1], // line 2  - top
  [3, 3, 3], // line 3  - bottom
...
// Win table: 9 symbols, must match init.json win_table
...
const SCATTER_SYMBOL = 9; // star
const SPECIAL_SYMBOLS = [1, 9]; // seven and star: max 1 per column
...
app.listen(PORT, () => {
  console.log(`\n  Fire Rage running at http://localhost:${PORT}\n`);
